...
def format_name(raw_artist, raw_name, type="song"):
	...
	# This accounts for stuff like Naïve" -> "Naive" and "Tournée" -> "Tournee"
	regex = re.compile(r'[ ./+]')
...
# out should take one of three values:
	# out = "std"  : Output to standard output (takes this value by default)
...
def main():
	...
	if args.item == "song":
		...
		site = "{GS}{name}-lyrics".format(GS=GENIUS_SITE, name=name) # Needs the http, else will fail
